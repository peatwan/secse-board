...
class Config:
    def __init__(self, config_file: str):
        ...
        try:
            ...
            if not read_files:
                raise ConfigError(f"Configuration file '{config_file}' not found or is empty.")

            # DEFAULT
            self.defaults = DefaultConfig(
                project_code=self.config_parser.get("DEFAULT", "project_code"),
                workdir=self.config_parser.get("DEFAULT", "workdir"),
                fragments=self.config_parser.get("DEFAULT", "fragments"),
                num_gen=self.config_parser.getint("DEFAULT", "num_gen"),
                num_per_gen=self.config_parser.getint("DEFAULT", "num_per_gen"),
                seed_per_gen=self.config_parser.getint("DEFAULT", "seed_per_gen"),
                start_gen=self.config_parser.getint("DEFAULT", "start_gen"),
                docking_program=self.config_parser.get("DEFAULT", "docking_program"),
                cpu=self.config_parser.getint("DEFAULT", "cpu"),
                gpu=self.config_parser.getint("DEFAULT", "gpu"),
                rule_db=self.config_parser.get("DEFAULT", "rule_db")
            )
            ...
            if self.config_parser.has_section("docking"):
                ...
                if "vina" in self.defaults.docking_program.lower():
                    ...
                    docking.box_size_x = self.config_parser.getfloat("docking", "box_size_x")
                    docking.box_size_y = self.config_parser.getfloat("docking", "box_size_y")
                    docking.box_size_z = self.config_parser.getfloat("docking", "box_size_z")
                self.docking = docking
            else:
                self.docking = None

            # DEEP LEARNING
            if self.config_parser.has_section("deep learning"):
                self.deep_learning = DeepLearningConfig(
                    mode=self.config_parser.getint("deep learning", "mode"),
                    dl_per_gen=self.config_parser.getint("deep learning", "dl_per_gen"),
                    dl_score_cutoff=self.config_parser.getfloat("deep learning", "dl_score_cutoff")
                )
            else:
            ...
            if self.config_parser.has_section("properties"):
                self.properties = PropertiesConfig(
                    ...
                    chiral_center=self.config_parser.getint("properties", "chiral_center"),
                    heteroatom_ratio=self.config_parser.getfloat("properties", "heteroatom_ratio"),
                    rdkit_rotatable_bound_num=self.config_parser.getint("properties", "rdkit_rotatable_bound_num"),
                    keen_rotatable_bound_num=self.config_parser.getint("properties", "keen_rotatable_bound_num"),
                    rigid_body_num=self.config_parser.getint("properties", "rigid_body_num"),
                    ...
                    lipinski_violation=self.config_parser.getint("properties", "lipinski_violation"),
                    QED=self.config_parser.getfloat("properties", "QED"),
                    max_ring_size=self.config_parser.getint("properties", "max_ring_size"),
                    max_ring_system_size=self.config_parser.getint("properties", "max_ring_system_size"),
                    ring_system_count=self.config_parser.getint("properties", "ring_system_count"),
                    bridged_site_count=self.config_parser.getint("properties", "bridged_site_count"),
                    spiro_site_count=self.config_parser.getint("properties", "spiro_site_count"),
                    fused_site_count=self.config_parser.getint("properties", "fused_site_count"),
                    rdkit_sa_score=self.config_parser.getint("properties", "rdkit_sa_score"),
                    substructure_filter=self.config_parser.getint("properties", "substructure_filter")
                )
            else:
    ...
    def set_value(self, section: str, option: str, value):
        ...
        """
        try:
            if not self.config_parser.has_section(section) and section != "DEFAULT":
